...
public class Compass2 extends AppCompatActivity implements SensorEventListener {
    ...
    public class CustomDrawableView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (azimuth != null)
                canvas.rotate(-azimuth*360/(2*(float)Math.PI), centerx, centery);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (mGravity != null && mGeomagnetic != null) {
            ...
            if (success) {
                ...
                float azimuthInRadians = azimuth.floatValue();
                float azimuthInDegress = (float)Math.toDegrees(azimuthInRadians);
                if (azimuthInDegress < 0.0f) {
                    azimuthInDegress += 360.0f;
                }
                azimuth = orientation[0]; // orientation contains: azimuth, pitch and roll
                Log.i("yoad", "Compass2-->orientation(Radians): " + azimuthInRadians);
                Log.i("yoad", "Compass2-->orientation(Degrees): " + azimuthInDegress);
            }
        }
